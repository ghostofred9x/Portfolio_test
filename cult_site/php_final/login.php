...
// Connects to your Database
	...
	$check = mysql_query("SELECT * FROM pjl_users WHERE username = '".$_POST['username']."'") or die(mysql_error());
	
	//Gives error if user doesn't exist
	$check2 = mysql_num_rows($check);
	if ($check2 == 0) {
		$error = 'That user does not exist in our database. <a href=index.php>Click Here to Register</a>';
	}
	while($info = mysql_fetch_array($check)) {
	...
	$page=$PHP_SELF.'?'.$_SERVER['QUERY_STRING'].'action=passerror';
...
<li>
<input type="submit" name="submit" value="Login" class="login1"/> </form>
</li>
<li>
<div id="error"><?php if($_GET['action'] == 'passerror'){
	
	$error = 'Your Username or Password seems to be incorrect';}
